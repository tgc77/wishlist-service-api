...
class ClientRepository(Repository):
    ...
    async def get_all(self) -> List[ClientEntity]:
        try:
            ...
            logger.error(f"Oops! Something went wrong to get all clients: {ex}")
    ...
    async def get_by_id(self, id: int) -> ClientEntity:
        try:
            ...
            logger.error(f"Oops! Something went wrong to get client with id: {id} => {ex}")
            raise UnprocessableEntityException(
                error=ex
            )
    ...
    async def get_by_email(self, email: str) -> ClientEntity:
        try:
            ...
            logger.error(f"Oops! Something went wrong to get client with email: {email} => {ex}")
            raise UnprocessableEntityException(
                error=ex
            )

    async def create(self, client_create: ClientEntity) -> ClientEntity:
        try:
            client = await self.get_by_email(email=client_create.email)
            if client:
                raise RegisterAlreadyExists

        except NoResultFound:
            try:
                new_client = ClientModel(**client_create.model_dump())
                ...
                logger.error(f"Oops! Something went wrong creating new client: {ex}")
    ...
    async def update(self, id: int, client_update: ClientEntity) -> ClientEntity:
        try:
            ...
            logger.error(f"Oops! Something went wrong updating client: {ex}")
            raise RegisterNotFound(
                error=ex
            )
        except IntegrityError as ex:
    ...
    async def delete(self, id: int):
        try:
            ...
            logger.error(f"Oops! Something went wrong deleting client: {ex}")
            raise RegisterNotFound(
                error=ex
            )
        except IntegrityError as ex:
